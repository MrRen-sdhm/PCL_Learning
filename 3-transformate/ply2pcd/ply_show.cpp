...
int main()
{
    ...
    pcl::io::loadPolygonFilePLY("coke.ply", mesh);									//PCL利用VTK的IO接口，可以直接读取stl,ply,obj等格式的三维点云数据,传给PolygonMesh对象
    pcl::io::mesh2vtk(mesh, polydata);												//将PolygonMesh对象转化为vtkPolyData对象
    pcl::io::vtkPolyDataToPointCloud(polydata, *cloud);								//获取点云
    pcl::io::savePCDFileASCII("coke.pcd", *cloud);									//存储为pcd文件



    char strfilepath[256] = "coke.pcd";
